...
np.random.seed(0) # set seed for riproducibility

def _randomize_data(n_fields:int, m_storages:int) -> tuple:
...
def get_data(use:tuple) -> tuple:
    k_vehicles = 3
    Q_vehicle_capacity = 10
    facility = 'F'
    data = _randomize_data(*use)
    return facility, k_vehicles, Q_vehicle_capacity, *data
